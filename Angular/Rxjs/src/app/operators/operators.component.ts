...
import { EMPTY, Observable, defer, empty, from, fromEvent, generate, interval, merge, of, range, switchMap, throwError, timer } from 'rxjs';
 ...
 constructor() { 
  ...
  // Creation Operators
    ...
    // 2. Create
          ...
          const subscribe3 = hello.subscribe((val:any)=>console.log('create',val))
    ...
    // 4. from (convert an array, promise, or iterable to an observable)
           ...
           const fromSubscribe = arraySource.subscribe(val=>console.log('from',val))

           //output: 1,2,3,4,5

           const promiseSource = from(new Promise(res=>res('Hello World!')))
           const subscribe4 = promiseSource.subscribe(val=>console.log('from',val))
    ...
    // 5. fromEvent (turn a event to observables)
          ...
          const fromEventSubscribe =  source.pipe(switchMap(_=>timer(1000))).subscribe(val=>console.log('fromEvent',val))

          document.addEventListener('click',()=>console.log('Clicked!'))

    // 6. generate() [generate an observable sequence by running a state-driven loop producing]

         const generateSource = generate(2,
                                          x=> x<= 38,
                                          x=> x+2,
                                          x=> '*'.repeat(x))
          
         const generateSubscribe = generateSource.subscribe(val=>console.log('generate',val))

    // 7. interval (Emit numbers in sequence based on provided timeframe.)

          const intervalSource = interval(1000);

          const intervalSubscribe = intervalSource.subscribe(val=>console.log('interval',val))

          setTimeout(()=>{intervalSubscribe.unsubscribe()},10000)
    ...
    **** 
    ...
    // 7. range() (Emit a sequence of numbers within a specified range.)
        ...
        const rangeSubscribe = rangeSource.subscribe(val=>console.log('range',val))
    ...
    // 9. timer(initialDelay,scheduler)
            ...
            //output: 0
            const timerSubscribe = timerSource.subscribe(val => console.log('timer',val));
            ...
            setTimeout(()=>{timerSubscribe2.unsubscribe()},10000) // this will unsubscribe the timer after 10 seconds
